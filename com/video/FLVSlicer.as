...
/**
 * This library lets you slice an FLV file with the Adobe Flash Player 10.
...
package org.bytearray.video 
{
	...
	public class FLVSlicer implements IEventDispatcher
	{
		...
		private var seekableDictionary:Vector.<Tag> = new Vector.<Tag>();
		private var nonSeekableDictionary:Vector.<Tag> = new Vector.<Tag>();
		...
		private function init ( buffer:ByteArray ):void
		{
			...
			{		
				...
				} else if ( currentTag == FLVSlicer.VIDEO_TAG )
				{
					keyframe = (fb & 0xF0) >> 4;
					codecID = (fb & 0xF0) >> 4;
					if ( keyframe == 1 ) seekableDictionary.push ( new Tag ( time, offset ) );
					else nonSeekableDictionary.push ( new Tag ( time, offset ) );
		...
		public function slice ( start:int, end:int ):FLVSlice
		{
			...
			var duration:Number = endTag.time - startTag.time;
			_slice.length = memory;
			_slice.writeBytes(input, startTag.offset, endTag.offset-startTag.offset);
			updateMetaData(duration*.001);
			updateTimeStamp(_slice, startTag.time);
			var slice:FLVSlice = new FLVSlice ( _slice, duration );
			dispatchEvent( new SlicedEvent ( SlicedEvent.COMPLETE, _slice, startTag.time, endTag.time ) );
			return slice;
		}
		
		/**
		 * Merges muliple FLVSlice objects into a single FLV stream.
		 * Make sure the different slices comes from the same incoming FLV.
		 * @param slices
		 * 
		 */		
		...
		{	
			if ( slices.length > 0 )
			{
				...
				var startingTime:Number = 0;
				var bytes:ByteArray;
				for each ( var slice:FLVSlice in slices )
				{	
					bytes = new ByteArray();
					bytes.writeBytes(slice.stream);
					startingTime += updateTimeStamp(bytes, startingTime, true);
					_slice.writeBytes(bytes, memory);
					duration += slice.duration;
				}
				updateMetaData(duration*.001);
				dispatchEvent( new MergedEvent ( MergedEvent.COMPLETE, _slice, duration ) );
				return new FLVSlice ( _slice, duration );
			}
			return null;
		}
		...
		{	
			if ( seekableDictionary.length )	
			{
				var firstTag:Tag = seekableDictionary[0];
				var lastTag:Tag = seekableDictionary[seekableDictionary.length-1];
				...
				for each ( var item:Tag in seekable ? seekableDictionary : nonSeekableDictionary )
				{
