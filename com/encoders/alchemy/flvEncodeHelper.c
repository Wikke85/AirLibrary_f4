...
/*
Makes all image blocks that make up a frame in one flattened ByteArray plus an array of offset indicies, which 
will then be handed off to Flash for final assembly of the FLV video frame.
(Each image block needs to be compressed, which will be done on the Flash side, taking advantage of Flash Player's 'native' zlib compress routine)
(General strategy is to minimize context switching between Alchemy and Flash)
Argument 1: ByteArray which will take image data result
Argument 2: ByteArray of unsigned shorts that are indices of each of the image blocks
*/
static AS3_Val makeImageBlocks(void* self, AS3_Val args){
	...
	for (row = 0; row < rowMax; row++)	{
		...
		for (col = 0; col < colMax; col++) {
			...
			int yStart = height - (row * BLOCK_HEIGHT);
			...
			for (y = yStart-1; y >= yEnd; y--){
				int off = (y * width * 4) + (xStart * 4)  +  1;
				// "+1" means R byte instead of A byte
				int x;
